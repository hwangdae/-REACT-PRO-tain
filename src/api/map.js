...
const getSearch = async (CoordResponse, selectedCategoryName) => {
  const response = await axios.get(
    ...
    {
      headers: { Authorization: `KakaoAK ${process.env.REACT_APP_KAKAO_MAP_LOCAL_KEY}` }
    }
  );
...
const getCoord = async (coordX, coordY, selectedCategoryName) => {
  ...
  // 장소 검색
  const placesResponse = await getSearch(CoordResponse.data.documents[0], selectedCategoryName);

  // 행정구역 정확히 일치하는 것만 필터링해서 return
  const region_3depth_name = CoordResponse.data.documents[0].region_3depth_name.replace('동', '');
  return {
    coord: CoordResponse.data.documents[0],
    places: placesResponse.documents.filter((item) => item.address_name.includes(region_3depth_name))
  };
};
